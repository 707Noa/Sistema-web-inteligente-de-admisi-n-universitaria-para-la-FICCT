...
import {
  getGrupos, generarGruposAuto, toggleEstadoGrupo, getGrupo, deleteGrupo, updateGrupo,
} from '../services/coordinadorService'
import {
  FiEye, FiAlertCircle, FiCheckCircle, FiRefreshCw, FiZap, FiTrash2, FiEdit,
} from 'react-icons/fi'

const TURNOS = ['mañana', 'tarde', 'noche']
const TURNO_LABELS = { mañana: 'Mañana (08:00-12:00)', tarde: 'Tarde (13:00-16:00)', noche: 'Noche (16:00-20:00)' }
const MATERIAS_ORDEN = ['Computación', 'Física', 'Inglés', 'Matemáticas']
const TURNO_HORAS = {
  mañana: [['08:00', '09:00'], ['09:00', '10:00'], ['10:00', '11:00'], ['11:00', '12:00']],
  tarde: [['13:00', '14:00'], ['14:00', '15:00'], ['15:00', '16:00']],
...
export default function AdministrarGrupos() {
  ...
  // Modales: null | 'ver' | 'generar' | 'resultGenerar' | 'asignar' | 'resultAsignar' | 'confirmarEliminar' | 'editar'
  ...
  // Edición de grupo
  const [editForm, setEditForm] = useState({ id: null, codigo: '', turno: '', dias: [], cupo_maximo: 70, estado: 'activo', aula: '' })
  ...
  const openEditar = (g) => {
    setEditForm({
      ...
      dias: g.dias || ['lunes', 'martes', 'miercoles', 'jueves', 'viernes'],
      cupo_maximo: g.cupo_maximo || 70,
      estado: g.estado || 'activo',
      aula: g.aula && g.aula !== '-' ? g.aula : '',
    })
    setEditError(null)
    setModal('editar')
  }
  ...
  const handleEditSubmit = async (e) => {
    ...
    setEditError(null)
    try {
      await updateGrupo(editForm.id, editForm)
      showMsg('success', 'Grupo actualizado con éxito.')
      close()
      fetchGrupos()
    } catch (err) {
              ...
              <tr key={g.id}>
                ...
                <td>
                  <div className="table-actions">
                    ...
                    <button className="btn btn-outline btn-sm" onClick={() => openEditar(g)} title="Editar grupo">
                      <FiEdit />
                    </button>
                    <button
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.82rem' }}>
                  <thead>
                    <tr style={{ background: 'var(--gray-100)' }}>
                      <th style={{ padding: '8px 12px', textAlign: 'left' }}>Materia (Día)</th>
                  ...
                  <tbody>
                    {(detalle.horarios || []).map((h, idx) => {
                      return (
                        <tr key={idx} style={{ borderTop: '1px solid var(--gray-200)' }}>
                          <td style={{ padding: '8px 12px', textTransform: 'capitalize' }}>{h.materia_nombre} ({h.dia})</td>
                    ...
                    {(detalle.horarios || []).length === 0 && (
                      <tr>
                        <td colSpan={2} style={{ padding: '12px', textAlign: 'center', color: 'var(--gray-400)' }}>
                          No hay horarios generados para este grupo.
                        </td>
                      </tr>
                    )}
                  </tbody>
      ...
      {modal === 'editar' && (
        <div className="modal-overlay" onClick={close}>
          <div className="modal" style={{ maxWidth: 460 }} onClick={e => e.stopPropagation()}>
            <div className="modal-header">
              <span className="modal-title">Editar Grupo {editForm.codigo}</span>
              <button className="modal-close" onClick={close}>×</button>
            </div>
            <form onSubmit={handleEditSubmit}>
              <div className="modal-body">
                {editError && (
                  <div style={{ display: 'flex', gap: 8, alignItems: 'center', background: 'var(--danger-light)', color: '#991b1b', padding: '10px 14px', borderRadius: 'var(--radius)', marginBottom: 16, fontSize: '0.85rem' }}>
                    <FiAlertCircle style={{ flexShrink: 0 }} /> {editError}
                  </div>
                )}
                ...
                <div className="form-group">
                  ...
                  <input
                    type="text"
                    className="form-input"
                    placeholder="Ej. Aula 12, Planta Baja, etc."
                    value={editForm.aula}
                ...
                <div className="form-group">
                  ...
                  <input
                    ...
                    onChange={e => setEditForm({ ...editForm, cupo_maximo: parseInt(e.target.value) || 70 })}
                    required
                  />
                </div>
              ...
              <div className="modal-footer">
                <button type="button" className="btn btn-outline" onClick={close} disabled={editSaving}>
                  Cancelar
                </button>
