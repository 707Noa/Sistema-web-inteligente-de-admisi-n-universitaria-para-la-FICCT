          ...
          <div className="modal" style={{maxWidth:520}} onClick={e=>e.stopPropagation()}>
            ...
            <div className="modal-body">
              ...
              {form.turno && form.dias.length>0 && (
                <div style={{marginTop:16,padding:'12px 16px',background:'var(--gray-50)',borderRadius:'var(--radius)',border:'1px solid var(--gray-200)'}}>
                  <p style={{fontSize:'0.8rem',fontWeight:600,color:'var(--gray-600)',marginBottom:8}}>Horarios a generar automáticamente:</p>
                  {MATERIAS_ORDEN.map((m,i)=>{
                    const [h1,h2] = TURNO_HORAS[form.turno]?.[i]||['--:--','--:--']
                    return (
                      <div key={m} style={{display:'flex',justifyContent:'space-between',fontSize:'0.8rem',padding:'3px 0',borderBottom:'1px dashed var(--gray-200)'}}>
                        <span>{m}</span>
                        <span style={{color:'var(--primary)',fontWeight:600}}>{h1} – {h2}</span>
                      </div>
                    )
                  })}
                  <p style={{fontSize:'0.75rem',color:'var(--gray-500)',marginTop:6}}>× {form.dias.length} día(s): {form.dias.join(', ')}</p>
                </div>
              )}
            </div>
